...
### 9.1 Python architecture ratchets

Architecture configuration is owner-controlled local policy and cannot be set by
a job. For changed or created `.py` files, planning compares the virtual result
with the current bounded inventory. Module line counts and direct Python-file
counts in touched parent directories use ratchet semantics: an existing violation
may remain unchanged or improve, but new growth beyond a hard limit fails
planning. Patch-wide counts bound newly created Python files and newly populated
Python directories. Excluded and protected paths do not participate.

Findings use stable `ARCH001`, `ARCH002`, `ARCH010`, `ARCH011`, `ARCH020`, and
`ARCH021` codes. Reports are deterministic and bounded, and state when truncated.
The policy is structural only: it does not inspect source semantics, infer
...
### 12.5.1 `python_structure`

Fields:

- `path`: relative directory or `.py` file, default `.`;
- `max_files`: positive integer, default `300`;
- `max_symbols`: positive integer, default `2000`;
- `compact`: boolean, default `false`.

A file target must end in `.py`. Other file targets fail planning. Directory
traversal follows the same protected, ignored, symbolic-link, file-size, and
output policies as other audit actions.
...
With `compact: false`, output retains the existing
`patchshuttle.python_structure_collection.v1` collection schema and
`patchshuttle.python_structure.v1` per-file schema. It contains requested path,
glob, file and parse-error counts, import counts, available and reported symbol
counts, limit signals, top-level import records, and declaration records with
kind, qualified name, physical range, parameters, decorators, and bases.

With `compact: true`, output uses
`patchshuttle.python_structure_collection.compact.v1` and
`patchshuttle.python_structure.compact.v1`. Collection counts, parse errors, and
limit signals are retained. Per-file records retain file paths, import and symbol
counts, and symbol kind, qualified name, and physical range. Detailed import
records, parameters, decorators, and bases are omitted. Syntax errors remain
bounded and never include source excerpts.

The implementation uses the standard-library AST and must not import or execute
project code. Both modes exclude source values, call and reference resolution,
import resolution, inferred types, runtime behavior, caching, persistence,
historical aggregation, and automatic patch-scope selection. The action is
intended for declaration discovery followed by narrower `read_symbol` audits
and, when required, a separately guarded `replace_symbol` patch.

The declarative Python adapter is equivalent to the YAML action:
...
authorization-header, private-key, and sensitive Python assignment patterns.
Python assignment matching includes prefixed password, token, API-key,
access-key, secret-key, private-key, and one-line secret-collection values while
preserving safe annotations, same-name references, and loader calls.
PatchShuttle must not describe redaction as a guarantee that a log contains no
secrets.
...
- evidence-only `PYTHON_DISCOVERY_EVALUATION` sections in run and early
  failure logs, limited to current-job actions, bounded audit output volume,
  targeting styles, and relevant failure signals; no symbol index, token
  estimate, historical aggregation, or automatic implementation verdict.
- aggregate Python discovery match counts, result-limit signals, and action
  duration from existing bounded audit outputs; nested conventional dependency
  environments are excluded for newly initialized workspaces, while existing
  local policy and project-specific backup exclusions remain owner-controlled.
- bounded declaration-only `python_structure` audit discovery implemented with
